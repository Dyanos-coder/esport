...
const initialValues: FormValues = {
  email: '', password: '', countryId: '', pseudo: '', fullName: '', phone: '', level: '', message: '',
};
...
export default function RegisterPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setStatus('loading');
    const result = await signup({ ...values, countryId: Number(values.countryId) });
    if (result.ok) {
      navigate('account');
      return;
    }
    setError(result.error || t('register.form.error'));
    setStatus('error');
  };
        ...
        <div className="container-x grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 glass-card p-6 md:p-10">
            ...
            <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <label><span className="label-field">Email</span><input required type="email" value={values.email} onChange={(e) => update('email', e.target.value)} placeholder="ton@email.com" className="input-field" /></label>
              <label><span className="label-field">Mot de passe</span><input required type="password" minLength={8} value={values.password} onChange={(e) => update('password', e.target.value)} placeholder="8 caractères minimum" className="input-field" /></label>
              <label><span className="label-field">{t('register.form.country')}</span><select required value={values.countryId} onChange={(e) => update('countryId', e.target.value)} className="input-field"><option value="">{t('register.form.country.placeholder')}</option>{countries.map((country) => <option key={country.id} value={country.id}>{country.flag} {country.name}</option>)}</select></label>
              ...
              {status === 'error' && <div className="md:col-span-2 rounded-xl bg-error-500/10 border border-error-500/30 p-4 text-sm text-error-400">{error}</div>}
              <div className="md:col-span-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-2">
                <div className="flex items-center gap-2 text-xs text-gray-500"><ShieldCheck className="w-4 h-4 text-success-400" /> Tes données restent confidentielles.</div>
                <button disabled={status === 'loading'} type="submit" className="btn-primary w-full sm:w-auto">{status === 'loading' ? t('register.form.submitting') : t('register.form.submit')}<Send className="w-4 h-4" /></button>
              </div>
              <p className="md:col-span-2 text-sm text-gray-400">
                Déjà un compte ? <button type="button" onClick={() => navigate('login')} className="text-lime-500 font-semibold hover:text-lime-400">Se connecter</button>
              </p>
            </form>
          </div>
